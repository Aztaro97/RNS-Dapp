		...
		async (
			...
			switch (chainType) {
				...
				case CustomChainType.router:
					...
					if (!routerNetworkEnv) {
						throw new Error(
							`networkEnv is undefined for router chain transaction`
						);
					}
					if (!routerContractAddress) {
						throw new Error(
							`contractAddress is undefined for router chain transaction`
						);
					}
					if (!routerExecuteMsg) {
						throw new Error(
							`executeMsg is undefined for router chain transaction`
						);
					}
					if (!routerNodeUrl) {
						throw new Error(
							`nodeUrl is undefined for router chain transaction`
						);
					}
				...
				default:
					throw new Error(`${chainType} chain type is not handled`);
			}
		},
		[accountAddress, chainType]
	);

	// const handleQueryTransaction = useCallback(async (txArgs:
	// 	| RouterExecutionType
	// 	| EthereumSendTransactionArgs
	// 	| NearExecutionType) => {
